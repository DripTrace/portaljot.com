...
// 		// Step 4: Download video directly to disk (streaming to avoid memory overhead)
// 		console.log("Downloading generated video");
// 		const videoStream = await axios.get(videoResult.videoUrl, {
// 			responseType: "stream",
// 		});
// 		const videoWriteStream = writeFile(videoPath, videoStream.data);

// 		await videoWriteStream;
...
// 		// Step 7: Read and return the final video
// 		console.log("Reading final video file");
// 		const finalVideo = await readFile(outputPath);
...
export async function POST(request: NextRequest) {
	try {
		...
		console.log("Starting video generation with params:", metadata);
		...
		console.log("Cleaning up temporary files...");
		await cleanupTempFiles([videoPath, combinedAudioPath, outputPath]);
