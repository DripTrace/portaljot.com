		...
		<div className="rounded-lg overflow-hidden items-center bg-slate-800 m-[15px] flex flex-col relative min-h-[100px] p-2">
			<div className="flex flex-row justify-between w-full">
				<div className="text-white py-1 text-base text-left w-full">
					{ANIMATION_TYPE_TO_LABEL[(props.animation as any).type]}
				</div>
				<button
			...
			{isFadeAnimation(props.animation) && (
				<FadeAnimation animation={props.animation} />
			)}
			{(props.animation as AnimationWithType).type === "slideIn" ||
			(props.animation as AnimationWithType).type === "slideOut" ? (
				<SlideAnimation
					animation={
						props.animation as unknown as
							| SlideInAnimation
							| SlideOutAnimation
					}
				/>
