...
async function parseJsonData(
	...
	return {
		...
		image: jsonData.image_url
			? { url: jsonData.image_url, position: "first" }
			: undefined,
	};
}
		...
		contents: [
			{
				parts: [
					{
						text: `Analyze this ${duration_seconds}-second video and provide a detailed description of the visual content.`,
					},
					{
						inlineData: {
							mimeType: "video/mp4",
...
export async function generateAudioSegments(
	...
	for (const segment of segments) {
		const response = await axios.post(
			...
			{
				text: segment,
				duration_seconds: totalDuration / segments.length,
			},
			{
				headers: {
					"xi-api-key": process.env.ELEVENLABS_API_KEY,
...
export async function combineAudioSegments(
	...
	const tempPaths = audioSegments.map((_, i) => `/tmp/audio_${i}.mp3`);
	await Promise.all(
		audioSegments.map((buffer, i) => writeFile(tempPaths[i], buffer))
	);

	const concatFilePath = "/tmp/concat.txt";
	const concatContent = tempPaths.map((path) => `file '${path}'`).join("\n");
	await writeFile(concatFilePath, concatContent);
	...
	return new Promise<void>((resolve, reject) => {
		ffmpeg()
			...
			.outputOptions(["-c", "copy"])
			.on("error", (err) => {
				console.error("FFmpeg error during audio combination:", err);
				reject(err);
			})
			.on("end", (stdout, stderr) => {
				console.log("Audio combination completed:", { stdout, stderr });
				resolve();
			})
			.run();
	});
...
export async function combineVideoAndAudio(
	...
	return new Promise<void>((resolve, reject) => {
		ffmpeg()
			...
			.on("error", (err) => {
				console.error(
					"FFmpeg error during video/audio combination:",
					err
				);
				reject(err);
			})
...
export async function cleanupTempFiles(filePaths: string[]): Promise<void> {
	await Promise.all(
		filePaths.map((path) => unlink(path).catch(console.error))
	);
}
