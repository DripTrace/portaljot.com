...
import { RATIO_RESOLUTIONS, RatioKey } from "@/constants/modify/video-size";
...
export class VideoRenderer {
	...
	constructor() {
		this.size = this.extractSize({
			...
			resolution:
				RATIO_RESOLUTIONS[
					DEFAULT_RATIO as keyof typeof RATIO_RESOLUTIONS
					// ].preffered,
				].prefered,
		});
	}
	...
	setCanvasBox = (canvasBox: HTMLDivElement | null) => {
		...
		if (!innerBox || !canvas) {
			throw new Error(
				"innerBox and canvasBox must contain canvas element"
			);
		}
