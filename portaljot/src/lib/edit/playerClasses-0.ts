...
// Define a type for the dump output
type DumpOutput = {
	width: number;
	height: number;
	name: string;
...
export class RenderedLayer {
	...
	drawScaled(
		...
		const out_ratio =
			ctx_out.canvas.clientWidth / ctx_out.canvas.clientHeight;
		let ratio = 1;
		let offset_width = 0;
		let offset_height = 0;
		...
		ctx_out.drawImage(
			video
				? (ctx as HTMLVideoElement)
				: (ctx as CanvasRenderingContext2D).canvas,
			0,
			0,
			width,
			height,
