...
interface FileWithURI {
  ...
  export class RenderedLayer {
    ...
    init(player: PlayerClass, preview: HTMLDivElement) {
      ...
      this.title_div = this.preview.querySelector('.preview_title') as HTMLDivElement;
    ...
    render_time(ctx: CanvasRenderingContext2D, y_coord: number, width: number, selected: boolean) {
    ...
    drawScaled(
      ctx: CanvasRenderingContext2D | HTMLVideoElement,
      ctx_out: CanvasRenderingContext2D,
      video = false
    ) {
      const width = video
        ? (ctx as HTMLVideoElement).videoWidth
        : (ctx as CanvasRenderingContext2D).canvas.clientWidth;
      const height = video
        ? (ctx as HTMLVideoElement).videoHeight
        : (ctx as CanvasRenderingContext2D).canvas.clientHeight;
      ...
      ctx_out.drawImage(
        video ? (ctx as HTMLVideoElement) : (ctx as CanvasRenderingContext2D).canvas,
        0,
        0,
        width,
        height,
  ...
  export class MoveableLayer extends RenderedLayer {
    ...
    update(change: LayerChange, ref_time: number) {
      ...
      if (change.scale) {
        ...
        const delta_x = ((this.width * old_scale) - (this.width * new_scale)) / 2;
        const delta_y = ((this.height * old_scale) - (this.height * new_scale)) / 2;
    ...
    render_time(ctx: CanvasRenderingContext2D, y_coord: number, base_width: number, selected: boolean) {
...
export class PlayerClass {
    ...
    constructor() {
      ...
      this.aux_time = 0;
      this.canvas_holder = document.getElementById('canvas') as HTMLElement;
      this.time_holder = document.getElementById('time') as HTMLElement;
      this.time_scale = 1.0;
      ...
      this.time_canvas.addEventListener('pointerup', this.scrubEnd.bind(this));
      this.time_canvas.addEventListener('pointercancel', this.scrubEnd.bind(this));
      ...
      this.cursor_preview = document.getElementById('cursor_preview') as HTMLElement;
      this.cursor_canvas = this.cursor_preview.querySelector('canvas') as HTMLCanvasElement;
      ...
      this.cursor_text = this.cursor_preview.querySelector('div') as HTMLElement;
    ...
    setupEventHandlers() {
      this.canvas_holder.addEventListener('pointerdown', this.onPointerDown.bind(this));
      this.canvas_holder.addEventListener('pointermove', this.onPointerMove.bind(this));
      this.canvas_holder.addEventListener('pointerup', this.onPointerUp.bind(this));
      this.canvas_holder.addEventListener('pointercancel', this.onPointerUp.bind(this));
      this.canvas_holder.addEventListener('wheel', this.onWheel.bind(this), { passive: false });
    ...
    onPointerDown(ev: PointerEvent) {
      ...
      if (this.pointers.size === 1) {
        ...
        this.initial_distance = Math.hypot(ev2.clientX - ev1.clientX, ev2.clientY - ev1.clientY);
      }
    }
    ...
    onPointerMove(ev: PointerEvent) {
      ...
      if (this.gesturing && this.pointers.size === 2 && this.selected_layer instanceof MoveableLayer) {
        const [ev1, ev2] = Array.from(this.pointers.values());
        const current_distance = Math.hypot(ev2.clientX - ev1.clientX, ev2.clientY - ev1.clientY);
        const scale_change = current_distance / (this.initial_distance || current_distance);
    ...
    onWheel(ev: WheelEvent) {
      ...
      if (this.selected_layer instanceof MoveableLayer) {
        ...
        this.selected_layer.update({ rotation: rotation_change }, this.time);
      }
    }
    ...
    add(layer: RenderedLayer) {
      ...
      this.total_time = Math.max(this.total_time, layer.start_time + layer.total_time);
    ...
    split() {
      if (this.selected_layer) {
        ...
        if (index !== -1) {
          ...
          const remainingTime = originalLayer.total_time - (currentTime - originalLayer.start_time);
          if (remainingTime > 0) {
            // Adjust the original layer's total time
            originalLayer.total_time = currentTime - originalLayer.start_time;
              ...
              if (newLayer instanceof MoveableLayer) {
                ...
                newLayer.anchors = new Set(
                  Array.from(originalLayer.anchors).filter(
                    (idx) => idx >= originalLayer.getIndex(currentTime)
                  )
                );
              }
            } else {
