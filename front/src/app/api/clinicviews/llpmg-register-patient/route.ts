...
// app/api/clinicviews/llpmg/feedback/route.ts
"use server";
...
export async function POST(req: NextRequest) {
	try {
		// Dynamically import renderToString
		...
		const patientEmailHtml = ReactDOMServer.renderToString(
			createElement(LLPMGEmailTemplate, {
				name: patientName,
				email,
				phone,
		...
		const doctorEmailHtml = ReactDOMServer.renderToString(
			createElement(LLPMGEmailTemplate, {
				name: patientName,
				email,
				phone,
