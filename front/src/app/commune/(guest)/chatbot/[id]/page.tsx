...
function Chatbot({ params: { id } }: { params: { id: string } }) {
	...
	const [messages, setMessages] = useState<Message[]>([]);
	const [date, setDate] = useState("");
	const [chatbot, setChatbot] = useState<ChatbotType | null>(null);

	const { data: chatBotsData } = useQuery<GetChatbotsData>(GET_CHATBOTS);
	...
	useEffect(() => {
		if (chatBotsData) {
			const foundChatbot = chatBotsData.chatbotsList.find(
