...
export const useStripeElements = () => {
	const StripePromise = async () =>
		await loadStripe(
			process.env.NEXT_PUBLIC_STRIPE_PUBLISH_SCHOOL as string
		);
	...
	const onStripeConnect = async () => {
		try {
			...
			const account = await axios.get(
				// `${process.env.NEXT_PUBLIC_URL}/api/stripe/connect?groupid=${groupid}`,
				`${process.env.DOMAIN_URL_SCHOOLL}/api/stripe/connect?groupid=${groupid}`
			);
			console.log("THE STRIPE ACCOUNT[ONSTRIPECONNECT]: ", {
				...account,
			});
