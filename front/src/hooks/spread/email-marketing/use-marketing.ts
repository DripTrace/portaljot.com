...
} from "@/schemas/spread/marketing.schema";
...
export const useEmailMarketing = () => {
	...
	const onCreateCampaign = handleSubmit(async (values) => {
		console.log("THE VALUES IN THE CAMPAIGN[USE-MARKETING]: ", values);
		try {
			setLoading(true);
	...
	const onCreateEmailTemplate = SubmitEmail(async (values) => {
		console.log("THE EMAIL IN THE CAMPAIGN[USE-MARKETING]: ", values);
	...
	const onAddCustomersToCampaign = async () => {
		try {
			setProcessing(true);
			const customersAdd = await onAddCustomersToEmail(
				isSelected,
				campaignId!
			);
			if (customersAdd) {
	...
	const onBulkEmail = async (emails: string[], campaignId: string) => {
		console.log("THE BULKS IN THE CAMPAIGN[USE-MARKETING]: ", emails);
