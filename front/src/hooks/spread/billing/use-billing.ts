...
export const useStripe = () => {
	...
	const onStripeConnect = async () => {
		try {
			...
			const account = await axios.get(`/api/spread/stripe/connect`);
			if (account) {
...
export const useStripeCustomer = (amount: number, stripeId: string) => {
	...
	useEffect(() => {
		...
		console.log("THE SELECTED AMOUNT[USE-BILLING]: ", amount);
	}, [amount]);
	...
	const onMakePayment = async (e: React.MouseEvent) => {
		...
		try {
			...
			const { error, paymentIntent } = await stripe.confirmPayment({
				elements,
				confirmParams: {
					return_url: `${process.env.DOMAIN_URL_SPREAD}/spread/settings`,
				},
...
export const useSubscriptions = (plan: "STANDARD" | "PRO" | "ULTIMATE") => {
	...
	const [payment, setPayment] = useState<"STANDARD" | "PRO" | "ULTIMATE">(
		plan
	);
	const { toast } = useToast();
