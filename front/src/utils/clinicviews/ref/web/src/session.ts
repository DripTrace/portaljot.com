...
export class CommonSession {
	...
	private initializeEventEmitterMethods(): void {
		this.off = this.eventEmitter.off.bind(this.eventEmitter);
		this.on = this.eventEmitter.on.bind(this.eventEmitter);
		this.once = this.eventEmitter.once.bind(this.eventEmitter);
		this.addListener = this.eventEmitter.addListener.bind(
			this.eventEmitter
		);
	...
	public sendInfoAndReceiveResponse(
		command: Command,
		_options?: any
	): Promise<any> {
		...
		return new Promise((resolve, reject) => {
					...
					if (statusCode === 200) {
						const onInfo = (
							...
							if (
								...
								if (
									obj.response.result.code.toString() === "0"
								) {
			...
			(this as any).info({ requestDelegate, requestOptions });
		});
	}
}
 ...
 *
 * [Reference](https://github.com/onsip/SIP.js/blob/master/docs/api/clinicviews/sip.js.invitation.md)
 */
 ...
 *
 * [Reference](https://github.com/onsip/SIP.js/blob/master/docs/api/clinicviews/sip.js.inviter.md)
 */
...
export function patchWebphoneSession(
	...
	// Add all the methods from CommonSession
	session.sendInfoAndReceiveResponse =
		sendInfoAndReceiveResponse.bind(session);
	...
	session.stateChange.addListener((newState) => {
		switch (newState) {
			case SessionState.Establishing: {
				...
				session.emit?.(Events.Session.Establishing); // Using optional chaining
				break;
			}
			case SessionState.Established: {
				console.log("session established: >>> \n");
				stopPlaying(session);
				session.addTrack?.(); // Using optional chaining
				session.emit?.(Events.Session.Established); // Using optional chaining
				break;
			}
			case SessionState.Terminating: {
				...
				session.emit?.(Events.Session.Terminating); // Using optional chaining
				break;
			}
			case SessionState.Terminated: {
				console.log("session terminated: >>> \n");
				stopPlaying(session);
				session.emit?.(Events.Session.Terminated); // Using optional chaining
				break;
			}
		}
	});
	...
	if (session.userAgent.onSession) {
		console.log("user agent session: >>> \n", session);
		session.userAgent.onSession(session);
	}

	console.log("before session return: >>> \n", session);
...
function sendInfoAndReceiveResponse(
	this: WebPhoneSession,
	command: Command,
	_options?: any
): Promise<any> {
	...
	return new Promise((resolve, reject) => {
		const requestDelegate: OutgoingRequestDelegate = {
			onAccept: (response: IncomingResponse): void => {
				let timeout: ReturnType<typeof setTimeout> | undefined;
				const callId = response.message.callId; // Ensure callId is extracted here
				if (response.message.statusCode === 200) {
					const onInfo = (message: IncomingRequestMessage): void => {
						...
						if (
							...
							timeout && clearTimeout(timeout);
							this.off?.("RC_SIP_INFO", onInfo); // Using optional chaining
					...
					this.on?.("RC_SIP_INFO", onInfo); // Using optional chaining
				} else {
					reject(
						new Error(
							`The INFO response status code is: ${response.message.statusCode} (waiting for 200)`
...
function mute(this: WebPhoneSession, silent?: boolean): void {
	if (this.state !== SessionState.Established) {
		(this as any).logger.warn(
			"An active session is required to mute audio"
		);
		return;
	}
	if (this.muted) {
...
function unmute(this: WebPhoneSession, silent?: boolean): void {
	if (this.state !== SessionState.Established) {
		(this as any).logger.warn(
			"An active session is required to unmute audio"
		);
		return;
	}
	if (!this.muted) {
	...
	if (localStream && remoteStream && !this.mediaStatsStarted) {
		...
		this.mediaStreams.getMediaStats((report: RTPReport) => {
			if (this.userAgent.enableMediaReportLogging) {
				(this as any).logger.log(
					`Got media report: ${JSON.stringify(report)}`
				);
			}
...
async function blindTransfer(
	...
	const newTarget =
		typeof target === "string"
			? UserAgent.makeURI(
					`sip:${target}@${this.userAgent.sipInfo!.domain}`
				)!
			: target;
...
async function unhold(this: WebPhoneSession): Promise<void> {
	...
	// this.addTrack(
	//     this.media.remote as HTMLVideoElement,
	//     this.media.local as HTMLVideoElement
	// );
	this.addTrack?.(
		this.media?.remote as HTMLVideoElement,
		this.media?.local as HTMLVideoElement
...
async function accept(
	this: WebPhoneSession,
	_options: InvitationAcceptOptions = {}
): Promise<void> {
	const options = _options || {};
	...
	options.sessionDescriptionHandlerOptions.constraints = options
		...
		const options: SessionInviteOptions = {
			requestDelegate: {
				onAccept: async (response): Promise<void> => {
					...
					enableSenderTracks(
						session,
						!session.held && !session.muted
					);
					resolve();
				},
				onReject: (): void => {
					(session as any).logger.warn(
						"re-invite request was rejected"
					);
					enableReceiverTracks(session, !session.held);
					enableSenderTracks(
						session,
...
function setupUserAgentCoreEvent(session: WebPhoneSession) {
	...
	const userAgentCore: WehPhoneUserAgentCore =
		session.userAgent.userAgentCore;
