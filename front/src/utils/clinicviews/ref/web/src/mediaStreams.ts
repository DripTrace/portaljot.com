...
export default class MediaStreams {
    ...
    public constructor(session: WebPhoneSession) {
        this.mediaStreamsImpl = new MediaStreamsImpl(session);
        this.release = this.mediaStreamsImpl.release.bind(
...
export class MediaStreamsImpl {
    ...
    public constructor(session: WebPhoneSession) {
        ...
        if (!session) {
            throw new Error(
                `${this.ktag}: Cannot initialize media stream monitoring. Session is not passed`
            );
        }
        this.session = session;
        this.onPeerConnectionStateChange =
    ...
    public getMediaStats(
        ...
        if (!this.onRTPStat && !callback) {
            (this.session as any).logger.debug(
                `${this.ktag}: No event callback provided to call when media stats are generated`
            );
            return;
        }
        if (callback) {
    ...
    private onPeerConnectionStateChange() {
        ...
        const sessionDescriptionHandler = this.session
            .sessionDescriptionHandler as SessionDescriptionHandler;
        const state =
            sessionDescriptionHandler.peerConnection!.iceConnectionState;
        if (Object.hasOwn(ConnectionState, state)) {
            eventName = ConnectionState[state];
            if (this.onMediaConnectionStateChange) {
                this.onMediaConnectionStateChange(eventName, this.session);
            ...
            (this.session as any).logger.debug(
                `${this.ktag}: Unknown peerConnection state: ${state}`
            );
        }
        (this.session as any).logger.debug(
            `${this.ktag}: peerConnection State: ${state}`
        );
    }
        ...
        try {
            ...
            stats.forEach((stat: { [key: string]: any }) => {
                switch (stat.type) {
                    case "inbound-rtp":
                        Object.keys(stat).forEach((statName) => {
                            switch (statName) {
                                ...
                                case "fractionLost":
                                case "mediaType":
                                    (report.inboundRtpReport as any)[statName] =
                                        stat[statName];
                                    break;
                                case "roundTripTime":
                                    report.rttMs[statName] = stat[statName];
                    ...
                    case "outbound-rtp":
                        Object.keys(stat).forEach((statName) => {
                            switch (statName) {
                                ...
                                case "packetsSent":
                                case "mediaType":
                                    (report.outboundRtpReport as any)[
                                        statName
                                    ] = stat[statName];
                                    break;
                            }
                    ...
                    case "media-source":
                        report.outboundRtpReport.rtpLocalAudioLevel =
                            stat.audioLevel ? stat.audioLevel : 0;
                        break;
                    case "track":
                        if (!stat.remoteSource) {
                            break;
                        }
                        report.inboundRtpReport.rtpRemoteAudioLevel =
                            stat.audioLevel ? stat.audioLevel : 0;
                        break;
                    case "transport":
                        Object.keys(stat).forEach((statName) => {
            ...
            if (!Object.hasOwn(report.rttMs, "currentRoundTripTime")) {
                if (!Object.hasOwn(report.rttMs, "roundTripTime")) {
                    report.rttMs.currentRoundTripTime =
                        this.preRTT.currentRoundTripTime;
                } else {
                    report.rttMs.currentRoundTripTime =
                        report.rttMs.roundTripTime; // for Firefox
                    delete report.rttMs.roundTripTime;
                }
            } else {
            ...
            (this.session as any).logger.error(
                `${this.ktag}: Unable to get media stats: ${(e as any).message}`
            );
        }
    }
}
