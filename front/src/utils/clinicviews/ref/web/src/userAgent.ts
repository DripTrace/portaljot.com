    ...
    await this.registerer!.register({
        requestDelegate: {
            onReject: (response): void => {
                ...
                if (
                    this.transport.isSipErrorCode!(response.message.statusCode)
                ) {
...
function invite(
    ...
    console.log("Inviting:", number, options);

    if (!number || typeof number !== "string") {
        throw new Error("Invalid number provided for invitation");
    }

    if (!this.sipInfo || !this.sipInfo.domain) {
        throw new Error("SIP info or domain is missing");
    }
    ...
    const uri = UserAgent.makeURI(`sip:${number}@${this.sipInfo.domain}`);
    if (!uri) {
        throw new Error(`Failed to create a valid URI for ${number}`);
    }
    ...
    inviter
        .invite({
            requestDelegate: {
                onAccept: (inviteResponse) => {
                    console.log("SESSION ACCEPTED >>>> ", inviteResponse);
                    inviter.startTime = new Date();
                    inviter.emit!(
                        Events.Session.Accepted,
                        inviteResponse.message
