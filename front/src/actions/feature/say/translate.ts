...
import { State } from "@/components/say/TranslationForm";
import connectDB from "@/config/say/mongodb/db";
import { addOrUpdateUser } from "@/config/say/mongodb/models/User";
...
async function translate(prevState: State, formData: FormData) {
	...
	const response = await axios({
		...
		params: {
			"api-version": "3.0",
			from:
				rawFormData.inputLanguage === "auto"
					? null
					: rawFormData.inputLanguage,
