...
export default function CustomerInfo() {
    ...
    const submitValidationHandler = async (
        validationData: Record<string, any>
    ) => {
        if (!session) return;

        await axios.post("/api/merchandise/stripe/create-customer", {
            firebaseID: session.id,
            validationData,
        });
    };

    const addingCard = async (e: React.FormEvent<HTMLButtonElement>) => {
        e.preventDefault();
    };

    const externalAccount = async (e: React.FormEvent<HTMLButtonElement>) => {
        e.preventDefault();
            ...
            <div className="absolute top-[50%] left-[50%] transform translate-x-[-50%] translate-y-[-50%]">
                ...
                <div className="relative top-0 left-0 w-[400px] min-h-[400px] bg-white/10 border rounded-[10px] flex justify-center align-center backdrop-blur-[5px] shadow-glass3 border-bottom-right-glass border-white/50">
                    <div className="relative w-full h-full p-[40px]">
                        ...
                        <CustomerShipping
                            onSubmitValidation={submitValidationHandler}
                        />
                    </div>
                </div>
            </div>
