...
const storiesPath = "public/imagine/stories";

import renderEventMessage from "@/lib/imagine/renderEventMessage";
...
function StoryWriter() {
			...
			eventData.forEach((data) => {
				try {
					...
					if (parsedData.type === "callProgress") {
						setProgress(
							parsedData.output[parsedData.output.length - 1]
								.content
						);
	...
	async function runScript() {
		...
		const response = await fetch("/api/imagine/run-script", {
			method: "POST",
			headers: {
	...
	useEffect(() => {
		if (runFinished) {
			toast.success("Story generated successfully!", {
				action: (
					<Button
						onClick={() => router.push("/imagine/stories")}
