...
const PokemonCard = ({
	...
	return (
		<div
			...
			} border shadow-md rounded-lg p-4 w-full h-80 overflow-hidden`}
		>
			...
			>
				{getTypeStyles(type).emoji} {revealed ? pokemon.name : "???"}
			</h2>

			<div className="flex-grow flex flex-col justify-between">
				<div className="text-center">
					{revealed ? (
						<p className={`text-sm ${getTypeStyles(type).text}`}>
							{pokemon.description}
						</p>
					) : (
						<button
			...
			<div className="w-full mt-auto">
				<span className="text-xs font-bold text-purple-600 block text-center">
					{rank}
				</span>
			</div>
		</div>
...
export default function QuizPage() {
          ...
          ${Object.entries(topPokemon)
				.map(
					([type, pokemon]) =>
						...
						} type: ${pokemon.name}: ${pokemon.traits.join("; ")}`
				)
				.join("\n\n")}
	...
	const handleShareResults = async () => {
		...
		const preparePokemons = (pokemons: Pokemon[]) =>
			pokemons.map(({ name, description, traits }) => ({
				name,
				description,
				traits,
			}));
