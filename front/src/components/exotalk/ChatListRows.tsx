...
import type { ChatMembers } from "@/lib/exotalk/converters/ChatMembers"; // or wherever you define this interface
import ChatListRow from "./ChatListRow";
import CreateChatButton from "./CreateChatButton";
import { MessageSquare } from "lucide-react";

function ChatListRows({ initialChats }: { initialChats: ChatMembers[] }) {
	// If there are no chat memberships yet, show the welcome UI
	...
	// Otherwise, render a row for each chat membership
	return (
		<div>
			{initialChats.map((member) => (
				<ChatListRow key={member.chatId} chatId={member.chatId} />
			))}
		</div>
	);
