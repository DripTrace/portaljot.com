...
import SipConvoComponent from "@/components/clinicviews/LLPMG/sip/SipConvoComponent";
...
const SipConvoContent: React.FC = () => {
	...
	useEffect(() => {
		...
		if (storedTokenData) {
			...
			if (tokenDataParam) {
				...
				setTokenData(parsedTokenData);
				router.replace("/clinicviews/llpmg/sip-convo"); // Redirect without the token in the URL
			}
	...
	const initiateAuth = useCallback(async () => {
		...
		try {
			const response = await fetch("/api/clinicviews/llpmg/sip-auth");
			if (!response.ok) {
		...
		async (code: string, state: string) => {
			...
			try {
				const response = await fetch(
					"/api/clinicviews/llpmg/sip-auth",
					{
						method: "POST",
						headers: {
				...
				if (data.tokenData) {
					...
					setIsAuthenticating(false);
					router.replace("/clinicviews/llpmg/sip-convo"); // Redirect to the page after storing the token
				} else {
	...
	useEffect(() => {
		...
		if (rcTokenData) {
			...
			if (tokenDataParam) {
				...
				setTokenData(parsedTokenData);
				router.replace("/clinicviews/llpmg/sip-convo");
			} else {
