...
import {
	...
	addSystemMessage,
	updateConversation,
} from "@/lib/clinicviews/llpmg/redux/reducers/convoReducer";
import { saveConversation } from "@/lib/clinicviews/llpmg/idb/convoDB";
...
const SipConvoComponent: React.FC<SipConvoComponentProps> = ({
	...
	const initializeWebPhone = useCallback(async (tokenData: any) => {
		try {
			const sipProvisionResponse = await fetch(
				"/api/clinicviews/llpmg/sip-provision",
				{
					method: "POST",
			...
			const options: WebPhoneOptions = {
				...
				audioHelper: {
					enabled: true,
					incoming: "/clinicviews/llpmg/audio/incoming.ogg",
					outgoing: "/clinicviews/llpmg/audio/outgoing.ogg",
				},
			};
	...
	const handleConversationAnalysis = useCallback(async () => {
		...
		try {
			...
			const analyzeResponse = await fetch(
				"/api/clinicviews/llpmg/analyze-conversation",
				{
					method: "POST",
					headers: { "Content-Type": "application/json" },
			...
			const generatedResponse = await fetch(
				"/api/clinicviews/llpmg/generate-conversation",
				{
					method: "POST",
					headers: { "Content-Type": "application/json" },
	...
	const generateGreeting = useCallback(async (callerNumber: string) => {
		try {
			const response = await fetch(
				"/api/clinicviews/llpmg/generate-greeting",
				{
					method: "POST",
					headers: { "Content-Type": "application/json" },
					body: JSON.stringify({
	...
	const transcribeAudio = useCallback(async () => {
		...
		recorder.onstop = async () => {
			...
			const response = await fetch(
				"/api/clinicviews/llpmg/speech-to-text",
				{
					method: "POST",
					headers: { "Content-Type": "application/json" },
