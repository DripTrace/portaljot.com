...
import SipComponent from "@/components/clinicviews/LLPMG/sip/SipComponent";
	...
	useEffect(() => {
		...
		if (storedTokenData) {
			...
			if (tokenDataParam) {
				...
				setTokenData(parsedTokenData);
				router.replace("/clinicviews/llpmg/sip"); // Redirect without the token in the URL
			}
		...
		async (code: string, state: string) => {
			...
			try {
				...
				if (data.tokenData) {
					...
					setIsAuthenticating(false);
					router.replace("/clinicviews/llpmg/sip"); // Redirect to the page after storing the token
				} else {
	...
	useEffect(() => {
		...
		if (rcTokenData) {
			...
			if (tokenDataParam) {
				...
				setTokenData(parsedTokenData);
				router.replace("/clinicviews/llpmg/sip");
			} else {
