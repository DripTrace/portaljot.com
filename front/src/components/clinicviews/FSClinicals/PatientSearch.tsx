...
const PatientSearch: React.FC = () => {
	...
	const handleSearch = async () => {
		...
		try {
			const response = await fetch(
				`/api/clinicviews/patient-search/route?query=${encodeURIComponent(searchQuery)}`
			);
			if (!response.ok) {
			...
			setError(
				"An error occurred while searching patients. Please try again."
			);
		} finally {
			setIsLoading(false);
		}
	...
	return (
		<div
			className={`p-4 ${isDarkMode ? "bg-[#0C3C60] text-[#D1E0EB]" : "bg-[#D1E0EB] text-[#0C3C60]"}`}
		>
			...
			<div>
				{searchResults.map((patient) => (
					<div
						key={patient.id}
						className="mb-4 p-4 rounded-lg bg-opacity-20 bg-white"
					>
