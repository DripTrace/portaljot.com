		...
		const fetchPatientDetails = async () => {
			try {
				const response = await fetch(
					`/api/clinicviews/patient-details?id=${patientId}`
				);
				if (!response.ok) {
		...
		>
			...
			<div className="mb-6">
				...
				{patient.addresses && patient.addresses[0] && (
					<p>
						Address:{" "}
						{`${patient.addresses[0].street}, ${patient.addresses[0].city}, ${patient.addresses[0].state} ${patient.addresses[0].postalCode}`}
					</p>
				)}
			</div>
