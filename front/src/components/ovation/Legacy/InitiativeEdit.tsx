...
const EditInitiativeModal: React.FC<EditInitiativeModalProps> = ({
    ...
    const [localFormData, setLocalFormData] = useState(currentItem);
    ...
    useEffect(() => {
        setLocalFormData(currentItem);
        setCurrentCategory(currentItem?.category || "");
    }, [currentItem]);
    ...
    const handleChange = (
        ...
        setLocalFormData((prevState: ItemType | null) => ({
            ...(prevState as ItemType),
            [name]: value,
        }));
    };
    ...
    const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
        ...
        if (
            localFormData &&
            localFormData.name &&
            "description" in localFormData &&
            "category" in localFormData &&
            "initiative_id" in localFormData &&
            validateForm(localFormData as FormData)
        ) {
            setIsModalOpen(false);

            const response = await fetch(
                `/api/initiative/${localFormData.initiative_id}`,
                {
                    method: "PUT",
                    headers: {
            ...
            >
                ...
                <form onSubmit={handleSubmit}>
                    ...
                    {categories.map((category) => {
                        ...
                        console.log("Is current category:", isCurrentCategory);
