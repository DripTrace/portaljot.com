...
import { Story, Page } from "@/types/imagine/stories";
import cleanTitle from "./cleanTitle";

const storiesDirectory = path.join(process.cwd(), "public/imagine/stories");
	...
	const stories: Story[] = storyFolders.map((storyFolder) => {
		...
		files.forEach((file) => {
			...
			if (pageNumber) {
				...
				if (type === "txt") {
					pageMap[pageNumber].txt = fs.readFileSync(
						filePath,
						"utf-8"
					);
				} else if (type === "png") {
					pageMap[pageNumber].png =
						`/imagine/stories/${storyFolder}/${file}`;
				}
			}
		});
