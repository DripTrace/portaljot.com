...
export class VideoTrackBuffer {
	...
	getVideoChunksDependencies = (timeInS: number) => {
		...
		const frameIndexAtTime = containingGroup.videoChunks.findIndex(
			(chunk) => {
	...
	getNextVideoChunks = (
		...
		const containingGroupIndex = this.videoChunksGroups.findIndex(
			(group) => {
	...
	private populateChunkGroups(samples: MP4Sample[]) {
		...
		for (const sample of samples) {
			...
			if (currentFramesGroup === null || frame.type === "key") {
				...
				currentFramesGroup.end = Math.max(
					currentFramesGroup.end,
					frameEndTs
				);
			}
		}
